...
def vsignin(request):            #vendor signin
    ...
    if request.method == 'POST':
        ...
        if v_form.is_valid():
            ...
            cpassword = v_form.cleaned_data.get('Confirm')
            adhar = v_form.cleaned_data.get('Aadhar')
            code = v_form.cleaned_data.get('Code')
            mobile = v_form.cleaned_data.get('PhoneNo')
            ...
            if User.objects.filter(email=Email).exists():
                ...
                return redirect('vsignin')
            elif len(password)< 8 and re.search('[A-Za-z0-9]',password) :
                messages.info(request,'Minimum length of password should be 8 or no special characters ')
                return redirect('vsignin')
            elif password != cpassword:
                messages.info(request,'Password does not match')
                return redirect('vsignin')
            elif len(adhar) != 16 and re.search('[0-9]',adhar):
                messages.info(request,"Enter Valid Adhar Number")
            else:
                v_form.save()
...
def csignin(request):            #customer signin
    ...
    if request.method == 'POST':
        ...
        if c_form.is_valid():
            ...
            cpassword = c_form.cleaned_data.get('Confirm')
            code = c_form.cleaned_data.get('Code')
            mobile = c_form.cleaned_data.get('PhoneNo')
            ...
            if User.objects.filter(email=Email).exists():
                ...
                messages.info(request,'Minimum length of password should be 8 and no special characters')
                return redirect('csignin')
            elif password != cpassword:
                messages.info(request,'Password does not match')
                return redirect('csignin')
            elif len(mobile)!= 10  and  re.search('[0-9]',mobile) :
                messages.info(request,"Enter Correct Mobile Number")
                return redirect('csignin')
            elif len(code)!= 6 and re.search('[0-9]',code):
                messages.info(request,"Enter Proper Postal Code")
                return redirect('csignin')
            else:
                c_form.save()
                user = User.objects.create_user(username=username,password=password,email=Email)
